...
{
	/* adresa element prelucrat = inceputul cozii */
	char *p = IC(a);
	/* contor */
  	size_t k = 0;
	/* pentru fiecare element, pana la ultimul */
...
{ 
	...
	/* actualizeaza SC pentru coada destinatie */
  	Avans(ad,SC(ad));
	/* actualizeaza IC pentru coada sursa */
	Avans (as,IC(as));
  	return 1;
}
