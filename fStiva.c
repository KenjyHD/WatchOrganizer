...
{ 
	char *temp = VF(a), *p, *q;
  	...
  	/* inverseaza perechi de elemente; 
	p, q pornesc de la baza, respectiv varful stivei */
...
{ 
	...
	if(VIDA(as)) 
  	...
  	/* mutare element */
	/* actualizeaza varful stivei sursa */
  	VF(as) -= DIME(as);
	/* muta elementul din varful stivei sursa */
  	memcpy(VF(ad), VF(as), DIME(as));
	/* actualizeaza varful stivei destinatie */
  	VF(ad) += DIME(ad);
  	return 1;
}
...
{ 
	size_t n = VF(as) - BS(as);
  	...
  	/* cat timp mai exista elemente in sursa -> muta din varf 
	sursa in varf destinatie */
 	 while(VF(as) > BS(as))
  	{
		/* actualizeaza varful stivei sursa */
		VF(as) -= DIME(as);  					
		/* muta elementul curent */
    	memcpy(VF(ad), VF(as), DIME(as)); 		
		/* actualizeaza varful stivei destinatie */
    	VF(ad) += DIME(as);						
  	}
